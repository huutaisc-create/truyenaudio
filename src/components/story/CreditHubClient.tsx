  ...
  return (
    <>
      <style>{`
        ...
        .ch-level-card{display:flex;flex-direction:column;align-items:center;gap:8px;padding:20px 18px 16px;border-radius:18px;background:linear-gradient(145deg,rgba(20,16,30,.95),rgba(12,10,20,.98));border:1.5px solid rgba(130,80,200,.3);box-shadow:0 0 24px rgba(130,80,200,.12);flex-shrink:0;min-width:148px}
        .ch-level-badge-top{font-size:12px;font-weight:700;color:rgba(255,255,255,.65);letter-spacing:.04em;white-space:nowrap}
        ...
        .ch-level-info{font-size:9.5px;color:rgba(255,255,255,.55);text-align:center;line-height:1.4}
        .ch-level-exp{font-size:10px;font-weight:700;font-family:var(--mono)}

        /* +3 CREDITS NOI BAT trong checkin header */
        .ch-bonus-badge{display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,rgba(245,166,35,.22),rgba(255,210,60,.1));border:1.5px solid rgba(245,166,35,.6);border-radius:10px;padding:4px 12px 4px 8px;font-size:14px;font-weight:800;color:var(--gold);letter-spacing:.02em;box-shadow:0 0 16px rgba(245,166,35,.2);animation:bonusGlow 1.8s ease-in-out infinite}

        @media(max-width:600px){.ch-hero-inner{flex-direction:column}.ch-realm-card,.ch-level-card{width:100%}}
        ...
        <div className="ch-page">
          ...
          <div className="ch-hero">
            ...
            <div className="ch-hero-inner">
              ...
              {/* CENTER: level card */}
              <div className="ch-level-card">
                <div className="ch-level-badge-top">{lv.icon} {lv.name}</div>
                ...
                <div className="ch-level-info" style={{ color: lv.color }}>
                  {lv.next ? `còn ${lv.toNext.toLocaleString('vi')} chương → ${lv.next.name}` : '✦ Đỉnh Phong'}
                </div>
                <div className="ch-level-exp" style={{ color: `${lv.color}99` }}>
                  {lv.pct}% tiến độ
                </div>
              </div>
