...
export default function ForYouSection({ stories, genrePrefs, showPicker }: Props) {
    const [modalOpen, setModalOpen] = useState(false)

    // Auto-open modal lần đầu vào trang
    useEffect(() => {
        if (showPicker) {
            // Delay nhẹ để trang load xong rồi mới pop modal
            ...
            <section id="section-foryou" className="scroll-mt-20" aria-label="Dành cho bạn">
                ...
                {!hasPrefs && (
                    ...
                    >
                        ...
                        <span className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-bold text-white"
                ...
                {hasPrefs && stories.length > 0 && (
                    <>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4">
                            {genrePrefs.map(g => (
                                <span key={g} className="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium"
