...
import { FACET_ORDER, type FacetType } from '@/lib/taxonomy'

export type SearchParams = {
    keyword?: string
    // Bộ lọc theo 7 nhóm (data-driven). Key = FacetType, value = danh sách tag đã chọn.
    ...
    if (facets) {
        for (const facet of FACET_ORDER) {
            ...
            if (vals && vals.length > 0) {
                andConditions.push(...vals.map(name => ({
                    genres: { some: { name: { contains: name }, type: facet } }
                })))
            }
        }
    }
        ...
        const [stories, total] = await Promise.all([
            db.story.findMany({
                ...
                select: {
                    ...
                    totalChapters: true,
                    genres: {
                        // Lấy đủ tag để thẻ truyện sắp theo thứ tự ưu tiên nhóm (FACET_ORDER)
