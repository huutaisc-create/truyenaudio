...
export default function ListeningClient({
  ...
  // ── Silent MP3 — Android Media Session lock screen controls ──
  const SILENT_MP3 = 'data:audio/mp3;base64,';
  const silentAudioRef = useRef<HTMLAudioElement | null>(null);

  // ── Chapter list: lazy load theo scroll ──
  ...
  useEffect(() => {
    ...
    if (!el || !desktopScrollEl) return;
    const obs = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting) {
        setDesktopVisible(prev => Math.min(prev + PAGE_SIZE, sortedChapters.length));
        loadMoreChapters();
      }
    }, { threshold: 0.1, root: desktopScrollEl });
    obs.observe(el);
    return () => obs.disconnect();
  }, [sortedChapters.length, desktopScrollEl, loadMoreChapters]);
  ...
  // ── Silent audio: sync isPlaying → Android lock screen controls ──
  useEffect(() => {
    if (!('mediaSession' in navigator)) return;
  ...
  useEffect(() => {
    return () => { silentAudioRef.current?.pause(); };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0f0d0a] relative">
      {/* ── Silent audio: Android Media Session lock screen controls ── */}
      <audio ref={silentAudioRef} src={SILENT_MP3} loop muted={false} style={{ display: 'none' }} />
      ...
      <div className="fixed bottom-16 right-3 z-50 pointer-events-none">
        <div className="bg-[#1a1612]/90 border border-white/[0.07] rounded-lg px-2 py-1">
          <span className="text-[10px] font-black text-[#e8580a]">v5.4</span>
        </div>
      </div>
      ...
      <div className="hidden lg:grid lg:grid-cols-12 h-screen">
        ...
        <div className="col-span-3 flex flex-col bg-[#0d0b08] border-l border-white/[0.06] h-screen">
          ...
          <div className="flex-1 overflow-y-auto" ref={setDesktopScrollEl}>
